...
$link.data("clicked",true);if(!keyword&&$link.find('img').length){imgsrc=$link.find('img').prop('src').split('index.html');keyword=imgsrc[imgsrc.length-1];}
...
return;href=href.replace('index.html','{protocol}').replace('index.html','{protocol}');var link_uri=href.replace(thirsty_global_vars.home_url,'').replace('{protocol}',''),link_prefix,new_href;link_uri=link_uri.indexOf('index.html')==0?link_uri.replace('index.html',''):link_uri;link_prefix=link_uri.substr(0,link_uri.indexOf('index.html')),new_href=href.replace('/'+link_prefix+'/','/'+thirsty_global_vars.link_prefix+'/').replace('{protocol}',window.location.protocol);return(link_prefix&&$.inArray(link_prefix,link_prefixes)>-1)?new_href:false;},linkFixer:function(){if(thirsty_global_vars.link_fixer_enabled!=='yes')
...
href=href.replace('index.html',window.location.protocol).replace('index.html',window.location.protocol);if(title)
